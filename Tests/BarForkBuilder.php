...
class BarForkBuilder implements ForkBuilderInterface
{
    private $filename;
    ...
    public function __invoke(PipelineRunnerInterface $runner, \Iterator $source): PipelineInterface
    {
        ...
        $pipeline
            ->transform(new Transformer\CallableTransformer(function($item) {
                $item['foo2'] = '42';
                return $item;
            }))
            ->load(new Loader\SplCSVLoader(
                new \SplFileObject($this->filename, 'w')
            ))
        ;
